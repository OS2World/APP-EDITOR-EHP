...
*                 Windows gesetzt. textline wird korrekt angepa·t.
*
*****************************************************************************/
...
*
*  Beschreibung : Die aktuelle Zeile wird nîtigenfalls aus dem Puffer in
*                 den Text Åbernommen. Bemerkung: alinep wird dabei nicht
*                 verÑndert, so da· Parameter GÅltigkeit behÑlt.
*                 Anschlie·end wird in den lokalen
*                 statischen Puffer eine Kopie der angegebenen Zeile
*                 gezogen, die jedoch keine
*                 Unterstreichung enthÑlt. Ist die aktuelle Zeile leer
...
zeil_typ *p;
{
  ...
  register int  sc;     /* Zaehler fuer Durchlaufen der Zeile */
  register char *zeile; /* Zeiger auf aktuellen Zeilentext    */
  static   char buf [BUFFSIZE]; /* Puffer, in dem Zeile zurÅckkommt */
...
int schreib_file ()
{
  ...
  {                   /* .bak erzeugen */
    ...
    if(sc = strchr(fn=sname(buff),'.')) /* Punkt im Filenamen suchen */
      len = sc-fn;      /* Falls gefunden, ist LÑnge die LÑnge bis zum Punkt */
    else                /* Sonst ist es die gesamte LÑnge des Filenamens */
      len = strlen(fn); /* Kann nicht lÑnger als 8 Zeichen sein. */
...
int lies_file()
{
  ...
  { /* Zeile einlesen, testen ob das ging und ob maximale */
	...
	akt_winp->changeflag = FALSE; /* Falls reserve_mem hÑngt, soll */
