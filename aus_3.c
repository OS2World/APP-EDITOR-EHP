...
*                 von dem dazugehîrigen einzusetzenden Zeichen in der Laenge
*                 (eins unterstrichen, das andere nicht), dann werden alle
*                 zu Åberschreibenden Zeichen gelîscht und fÅr die neuen
...
int *restn;
{
  ...
  char u_diff=FALSE;   /* Flag: Einzutragendes Zeichen und zu Åberschrei- */
		       /* bendes Zeichen unterschiedlichen Unterstrcih-Mode */
  int  anz_del=0,      /* Anzahl lîschbarer Zeichen */
  ...
  if(u_diff)  /* Hatten 2 Zeichen unterschiedliche LÑnge ? */
  {
    mdelete(anz_del);       /* Zu Åberschreibende Zeichen loeschen */
...
void do_movewin()
{
  ...
  do
  {
...
#ifdef OS2  /* Bei OS/2 kein zweites Zeichen lesen! */
    {
      ...
      {
	d = newwgetch(stdscr); /* Dann nÑchstes Zeichen lesen */
#endif
	for(i=0;i<step;i++)      /* verschieben */
      ...
      } /* Achtung!!! Diese Klammer gehîrt zum else fÅr OS2, zu if sonst */
...
int t;
{
  hide_show(MOUSE_HIDE);  /* Maus vor AusfÅhrung der Operation verstecken */
  (*funktion[t])();
